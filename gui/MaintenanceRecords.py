...
from function import maintenancerecords, globalfunction
...
window.title("維護紀錄")
window.protocol("WM_DELETE_WINDOW", lambda: maintenancerecords.on_closing(window))# 将窗口关闭事件绑定到 on_closing 函数
...
button_1 = Button(
    ...
    borderwidth=0,
    highlightthickness=0,
    command=lambda: maintenancerecords.back_to_recordpage(window),
    relief="flat"
)
button_1.place(
    x=14.0,
...
button_2 = Button(
    ...
    borderwidth=0,
    highlightthickness=0,
    command=lambda: maintenancerecords.open_maintenance_window(window),
    relief="flat"
)
button_2.place(
    x=34.0,
...
button_3 = Button(
    ...
    borderwidth=0,
    highlightthickness=0,
    command=lambda: maintenancerecords.delete(window),
    relief="flat"
)
button_3.place(
    x=631.9404296875,
...
#表格
# 定义列名
column_names = maintenancerecords.get_column_names()
# 从数据库获取数据
data_rows = maintenancerecords.get_table_value()
